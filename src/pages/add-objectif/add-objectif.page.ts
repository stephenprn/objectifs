...
export class AddObjectifPage {
  ...
  constructor(
    ...
    this.periodicitiesCustomJson = this.utilsService.getObjectFromArray(
      "id",
      ["title", "every"],
      AppConstants.customPeriodicities
    );
    this.periodicityCustom = _.cloneDeep(AppConstants.initialCustomPeriodicity);
    this.periodicitiesCustomDays = _.cloneDeep(
      AppConstants.customDaysPeriodicities
    );
  ...
  private patchForm(objectif: Objectif, laterMode?: boolean) {
    ...
    if (laterMode && objectif.dateEndPeriodicity != null) {
      dateEndPeriodicity = this.dateService.formatDateString(
        objectif.dateEndPeriodicity,
        true
      );
    }
    ...
    const alert: Alert = this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          text: "Ok",
          handler: (type: string) => {
            const periodicityNbr: string = (<HTMLInputElement>(
              this.document.getElementById("periodicityNbr")
            )).value;
  ...
  private getTitlePeriodicityCustom(): void {
    this.periodicityCustom.text =
      this.periodicitiesCustomJson[this.periodicityCustom.type].every +
      this.periodicityCustom.number +
      " " +
  ...
  saveDraft(): void {
    ...
    if (
      ...
      ((this.formGroup.value.title != null &&
        ...
        (this.formGroup.value.description != null &&
  ...
  submit(draft?: boolean): void | Objectif {
    ...
    if (this.formGroup.get("periodicity").value !== "punctual") {
      const beginDate: Date = this.dateService.getDateFromString(
        this.formGroup.value.date,
        true
      );
      const endDate: Date = this.dateService.getDateFromString(
        this.formGroup.value.dateEndPeriodicity,
        true
      );
    ...
    if (this.formGroup.get("periodicity").value === "customDays") {
      selectedDays = this.periodicitiesCustomDays.filter(
        (day: CustomDayPeriodicity) => {
          return day.selected;
        }
      );
    ...
    if (objectif.periodicity !== "punctual") {
      objectif.dateEndPeriodicity = this.dateService.formatDateString(
        objectif.dateEndPeriodicity
      );
    } else {
    ...
    if (objectif.periodicity === "customDays") {
      objectif.periodicityCustomDays = selectedDays
        .map((day: CustomDayPeriodicity) => {
          return day.id;
        })
        .sort();
    }
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: (data: any) => {
            if (!data) {
              this.uiService.displayToast(
                "Vous devez sélectionner un objectif"
              );
              return false;
            }
